...
// Function to get Telugu voice or fallback voice
function getTeluguCompatibleVoice() {
  // find Telugu voice
  let teluguVoice = voices.find(v => v.lang === 'te-IN');
  // fallback to Hindi or default voice if Telugu is not found
  if (!teluguVoice) {
    teluguVoice = voices.find(v => v.lang === 'hi-IN') || voices[0];
  }
  return teluguVoice;
}
...
function speak() {
  ...
  // Use selected voice or Telugu compatible voice if none selected
  if (voiceSelect.value) {
    currentUtterance.voice = voices[voiceSelect.value];
  } else {
    currentUtterance.voice = getTeluguCompatibleVoice();
  }
